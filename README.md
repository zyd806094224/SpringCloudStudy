...
### 死信消息表（dead_letter_message）
- 存储消费失败重试耗尽后的死信消息
- 用于死信消息的人工排查与定时重试补偿
- 详见 `sql/dead_letter_message.sql`
...
- [OpenFeign 使用与商用方案](docs/OpenFeign使用与商用方案.md)：集成方式与每层设计原因、打 feign 包（API 模块）模式、私服发布与版本治理、6 个实际商用场景、踩坑记录与选型总结
- [RocketMQ 环境接入与商用部署](docs/RocketMQ环境接入与商用部署.md)：本地服务端搭建（docker compose）、应用接入、brokerIP1 原理、多项目共享、商用部署架构与选型
- [V3 事务消息端到端链路](docs/V3事务消息端到端链路.md)：一条消息的完整旅程（发送→半消息→本地事务→回查→消费→幂等→重试→死信兜底）+ 实测数据 + 商用对比 + 场景选型（该不该用事务消息）
- [RocketMQ 使用与商用方案](docs/RocketMQ使用与商用方案.md)：事务消息、消费幂等、死信队列、告警机制与生产最佳实践
- [Gateway 使用与商用方案](docs/Gateway使用与商用方案.md)：网关作用与定位、集成方式（路由/负载均衡/过滤器）、请求转发链路原理、6 个商用场景、优劣势对比（Nginx/APISIX）、踩坑记录与生产注意事项
